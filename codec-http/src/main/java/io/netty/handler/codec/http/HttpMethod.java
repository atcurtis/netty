...
public class HttpMethod implements Comparable<HttpMethod> {
    ...
    private static final HttpMethod[] methodMap = new HttpMethod[1024];

    static {
        for (HttpMethod method : new HttpMethod[] {
            OPTIONS, GET, HEAD, POST, PUT, PATCH, DELETE, TRACE, CONNECT
        }) {
            int shortCode = shortCode(method.asciiName());
            assert shortCode > 0 && shortCode < 1024;
            assert methodMap[shortCode] == null;
            methodMap[shortCode] = method;
        }
    }
    ...
    public static HttpMethod valueOf(String name) {
        HttpMethod method = lookup(name);
        return method == null ? new HttpMethod(name) : method;
    }

    public static HttpMethod valueOf(CharSequence name) {
        HttpMethod method = lookup(name);
        return method == null ? new HttpMethod(AsciiString.of(name)) : method;
    }

    private static HttpMethod lookup(CharSequence name) {
        HttpMethod method = methodMap[shortCode(name)];
        if (method != null && method.asciiName().contentEqualsIgnoreCase(name)) {
            return method;
        }
        return null;
    }

    private static int shortCode(CharSequence name) {
        if (name.length() > 1) {
            int a = name.charAt(0);
            if (a >= 'a' && a <= 'z') {
                a -= 32;
